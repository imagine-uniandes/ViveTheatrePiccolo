...
namespace Phonon
{
	...
	{
        [DllImport("phononrv")]
        public static extern void iplInitializeReverb(IntPtr allocCallback, IntPtr freeCallback, IntPtr easyAllocCallback, IntPtr easyFreeCallback);

        [DllImport("phononrv")]
        public static extern void iplReleaseReverb();

        [DllImport("phononrv")]
        public static extern Error iplCreateReverbBaker(IntPtr scene, IntPtr grid, ReverbSettings settings, int typeFlags, [In, Out] ref IntPtr baker);

        [DllImport("phononrv")]
        public static extern void iplDestroyReverbBaker(IntPtr baker);

        [DllImport("phononrv", CallingConvention = CallingConvention.Cdecl)]
		public static extern Error		iplBakeReverb(IntPtr baker, IntPtr grid, BakeCallback callback);

		[DllImport("phononrv")]
		public static extern void		iplCancelReverbBake(IntPtr baker);
		
		[DllImport("phononrv")]
		public static extern Error		iplLoadBakedReverb(int numGridPoints, int dataSize, byte[] data, [In, Out] ref IntPtr baker);
		
		[DllImport("phononrv")]
		public static extern int 		iplSaveBakedReverb(IntPtr baker, byte[] data);
		
		[DllImport("phononrv")]
		public static extern IntPtr		iplGetBakedReverb(IntPtr baker, IntPtr grid, ReverbType type, Vector3 position);

...
public class PhononReverbPane
{
    ...
    static void DisplayProgressBar()
    {
        ...
        if (Status == ReverbBakeStatus.InProgress)
        {
            ...
            {
                Phonon.Reverb.iplCancelReverbBake(baker);
                BakeCanceled = true;
	...
	static void BakeThread()
	{
        ...
        Phonon.Reverb.iplCreateReverbBaker(AcousticSceneExporter.Scene, bakeGrid, bakeSettings, bakeTypeFlags, ref baker);
		Phonon.Reverb.iplBakeReverb(baker, bakeGrid, bakeCallback);
    }
    ...
    static void EndBake()
    {
		...
		else
		{
			...
			bakedData.SaveBakedReverb(baker);
    ...
    private static IntPtr baker = IntPtr.Zero;

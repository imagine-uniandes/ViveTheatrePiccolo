...
public class ReverbListener : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        Phonon.Reverb.iplInitializeReverb(IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero);
        if (Grid != null)
		{
			grid = Grid.LoadGrid();
    ...
    void OnDestroy()
    {
        ...
        Phonon.Reverb.iplReleaseReverb();

        // Shut down the effect.
		BakedData.UnloadBakedReverb();
		Grid.UnloadGrid();
    ...
    void Update()
    {
        ...
        if (Mode != ReverbMode.Off && Type == Phonon.ReverbType.Parametric)
        {
			if (audioEngine == Phonon.AudioEngine.Unity)
			{
				...
				reverbFilter.lfReference = parametricParams.lfReference;
#endif
                reverbFilter.roomRolloffFactor = parametricParams.roomRolloff;
    ...
    void SimulateReverb()
    {
        while (true)
        {
            try
            {
                ...
                {
					IntPtr parametricReverb = Phonon.Reverb.iplGetBakedReverb(BakedData.GetBaker(), grid, Phonon.ReverbType.Parametric, vListener);
                ...
                {
					IntPtr convolutionReverb = Phonon.Reverb.iplGetBakedReverb(BakedData.GetBaker(), grid, Phonon.ReverbType.Convolution, vListener);

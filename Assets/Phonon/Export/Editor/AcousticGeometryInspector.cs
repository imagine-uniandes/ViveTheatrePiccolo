...
//
// AcousticGeometryInspector
// Custom inspector for the AcousticGeometry class.
//

[CustomEditor(typeof(AcousticGeometry))]
public class AcousticGeometryInspector : Editor
{
    public override void OnInspectorGUI()
    {
        serializedObject.Update();

        AcousticGeometry geometry = serializedObject.targetObject as AcousticGeometry;

		Phonon.PhononGUI.SectionHeader("Geometry Statistics");
        EditorGUILayout.LabelField("Vertices", geometry.NumVertices.ToString());
        EditorGUILayout.LabelField("Triangles", geometry.NumTriangles.ToString());

		if (geometry.AttachedTerrain != null)
		{
			Phonon.PhononGUI.SectionHeader("Terrain Export Settings");
